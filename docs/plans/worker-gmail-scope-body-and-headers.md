...
- **`.spec/email-pipeline.spec.yml:67-72` EMAIL-007** — parse_failed 그레이스풀 디그레이드:
  > "HTML 파싱 실패(잘못된 HTML / charset mismatch / Jsoup timeout)는 raw event를 quarantine으로 넘기지 않고 event_snippet=subject로 graceful degrade한다. Firebase Crashlytics에만 parse_failure 이벤트 기록 … EmailBody.body_plain=null + body_html=원본 보존 + parse_failed=true 플래그. Firebase Crashlytics에 {error:'email_html_parse_failed', message_id, provider} 이벤트"
...
| parse_failed | HTML 파싱 실패 시 body_html 원본 보존 + parse_failed=true + Firebase Crashlytics | 브랜치 자체 없음 | try/catch Jsoup + flag + Firebase Crashlytics |
...
- `android/app/src/main/java/com/becalm/android/worker/ingestion/GmailWorker.kt`
  ...
  - HTML 파싱 실패 분기 (EMAIL-007):
    ...
    - `try { Jsoup.parse(bodyHtml).text() } catch (e: Throwable)` → `parseFailed = true`, `bodyPlain = null`, `bodyHtml = 원본 유지`, Firebase Crashlytics breadcrumb `{error:"email_html_parse_failed", message_id: messageId, provider: "gmail"}`, `eventSnippet = subject.take(200)` (graceful degrade).
...
- `android/app/src/test/java/com/becalm/android/worker/ingestion/GmailWorkerTest.kt`:
  ...
  - `htmlParseFailure_parseFailedTrue` — Jsoup parse throwable simulate (가짜 `EmailHtmlParser` stub) → EmailBody.parseFailed=true, body_html=원본, body_plain=null, Firebase Crashlytics breadcrumb enqueue 호출.
...
- **parse_failed Firebase Crashlytics breadcrumb**: 현재 프로젝트에 Firebase Crashlytics 통합이 있는지 `grep -rn "Firebase Crashlytics\." android/app/src/main/java/` 로 확인 필요. 없으면 본 PR 은 Firebase Crashlytics 호출을 stub 으로 두고 실제 wiring 은 별도 PR (`feat/infra/crashlytics`) 담당. `Logger.e(TAG, "email_html_parse_failed ...")` 로 local log 만 남기는 degrade 전략 OK.
