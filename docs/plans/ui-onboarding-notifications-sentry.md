# UI / Onboarding / notifications-sentry — POST_NOTIFICATIONS 권한 스텝 + Sentry `onboarding_step_failed` 인프라

**Branch**: `feat/ui/onboarding/notifications-sentry`
**Status**: PLAN ONLY — 구현은 별도 세션. 이 plan doc 이외의 코드 커밋 금지.
**E2E Stage**: 2 — Onboarding (POST_NOTIFICATIONS 권한, Sentry 관측 인프라)
...
### 1.1 Sentry 부재
- `grep -rn "io.sentry\\|Sentry\\." android/app/src/main/` 은 comment 1줄 외 0 matches. Sentry SDK 미의존, 초기화 코드 없음.
...
> "온보딩 실패 이벤트가 Sentry 에 전송된다. OAuth 인증 실패 또는 권한 거부 발생 시 Sentry 에 `onboarding_step_failed` 이벤트 전송됨 (step 이름, error 포함)"
...
> Sentry 에 전송되는 payload 에 이메일·토큰·본문 같은 PII 가 포함돼서는 안 됨.
...
### 3.1 Sentry 미의존
- `app/build.gradle.kts` 에 `io.sentry:sentry-android` 없음.
- `BecalmApplication.onCreate` 에 Sentry.init 없음.
...
검증 grep:
```bash
grep -rn "Sentry\\|io\\.sentry" android/app/
...
|------|-----------|-----------|------|
| Sentry SDK | 초기화됨 | 미의존 | build.gradle + init |
| Observability 래퍼 | `ObservabilityClient` (captureEvent, addBreadcrumb) 인터페이스 + Sentry impl | 없음 | 신규 |
...
### 5.1 Files to change
- **`app/build.gradle.kts`** — `implementation("io.sentry:sentry-android:7.+")` (버전은 lib 매뉴얼 검증). `buildConfigField("String", "SENTRY_DSN", "\"${env.SENTRY_DSN ?: ""}\"")`. Debug 빌드는 optional (DSN empty 면 `ObservabilityClient.Noop` 사용).
- **`BecalmApplication.kt`** — `onCreate` 에서 `ObservabilityClient.init(sentryDsn = BuildConfig.SENTRY_DSN)`. PIPA: `beforeSend` callback 에 email/token pattern regex strip.
- **`core/di/ObservabilityModule.kt`** — `@Provides @Singleton fun provideObservabilityClient(...): ObservabilityClient` — DSN 존재 시 `SentryObservabilityClient`, 아니면 `NoopObservabilityClient`.
...
- **`core/observability/ObservabilityClient.kt`** — interface. 4 메서드:
  ...
  ```
- **`core/observability/SentryObservabilityClient.kt`** — impl wrapping `io.sentry.Sentry`.
...
- **tests**:
  ...
  - `SentryObservabilityClientTest` — `beforeSend` 가 email 문자열을 strip 하는지 unit 테스트.
...
- **CI secret**: `SENTRY_DSN` — CTO 가 CI 에 등록 (본 plan 범위 밖).
- **Privacy policy**: Sentry 사용 고지 — 본 plan 범위 밖 (법무팀).
...
- [ ] **Unit test**: `SentryObservabilityClientTest.beforeSend_stripsEmailPattern` 통과
...
- **PIPA action log 에 Sentry event 동시 append** — W7 범위.
- **Crash reporting 자동 업로드 동의 UI** — 약관 화면 / spec amendment 범위.
- **Firebase Crashlytics** — Sentry 대체재. 선택은 이미 Sentry 로 확정 (ONB-007).
...
- **Blocked by**: 없음.
- **Blocks**:
  - S6-D/F/G/H (onboarding screens) — Sentry breadcrumb / captureMessage 를 본 plan 의 `ObservabilityClient` 로 dispatch.
...
- Sentry 의존성 제거 — 이미 수집된 event 는 Sentry 서버에 남음 (rollback 으로 삭제되지 않음).

---

## Appendix — Session handoff notes

- **왜 `ObservabilityClient` 인터페이스로 감싸는가**: (1) DSN 미설정 debug 빌드에서도 compile 가능, (2) 테스트에서 Fake 주입 용이, (3) Sentry → Firebase Crashlytics 로의 교체 비용 최소화. 간접비용은 class 1개.
- **`beforeSend` 에서 strip 해야 할 패턴**: `\\b[\\w.+-]+@[\\w-]+\\.[\\w.-]+\\b` (email), Authorization header, JWT pattern `ey[A-Za-z0-9_-]+\\.[A-Za-z0-9_-]+\\.[A-Za-z0-9_-]+`.
- **`setUserScope`**: Supabase userId UUID 는 고유하지만 PII 아님. Sentry scope 에 기록해도 PIPA 무위반 — 단 `accountEmail` 은 **금지**.
