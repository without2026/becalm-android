 ...
 * 메시지를 유지할 수 있도록 다음 3-파라미터 형태로 확정한다:
 *
 * - [logMessage] — `logger.e(tag, ...)`에 그대로 전달되는 로그 문자열.
 *   `RawIngestionRepository` 기존 호출부는 이를 `"<op> failed"` 형태로, `CommitmentRepository`는
 *   필요 시 `userId=$userId` 같은 동적 값을 포함한 임의 문자열로 전달한다.
 * - [fallbackMessage] — 원본 예외의 `message`가 null일 때 `BecalmError.Io`가 사용할 대체 문자열.
 *   호출자가 생략하면 [logMessage]를 그대로 사용하므로, 두 값이 동일한 Commitment 호출부는
 *   단일 인자로 축약된다.
 *
...
internal inline fun <T> Logger.daoOp(
    tag: String,
    logMessage: String,
    fallbackMessage: String = logMessage,
    block: () -> T,
): BecalmResult<T> = try {
    BecalmResult.Success(block())
} catch (ex: Exception) {
    e(tag, logMessage, ex)
    BecalmResult.Failure(BecalmError.Io(ex.message ?: fallbackMessage))
}
