# UI / Onboarding / outlook-oauth — OutlookMailOAuthScreen 을 실제 MSAL `acquireTokenInteractive` 흐름으로 교체 + 실패 시 Sentry `onboarding_step_failed`
...
이 plan 은 Gmail 용 plan (`ui-onboarding-gmail-oauth.md`) 과 **같은 설계 패턴**을 MSAL 에 적용. UI launcher 는 별도지만, VM 의 콜백 naming / Sentry / UserPrefsStore 쓰기 패턴은 동일하다. 토큰 공급자 구현은 PR #4 (`docs/plans/repo-auth-msgraph-token-provider.md`, 별도 작성 예정) 가 담당 — 본 plan 은 UI wiring 만.
...
> "온보딩 실패 이벤트가 Sentry에 전송된다 … 온보딩 중 OAuth 인증 실패 또는 권한 거부 발생 … Sentry에 `onboarding_step_failed` 이벤트 전송됨 (step 이름, error 포함)"
...
| 실패/거부 시 | Sentry `onboarding_step_failed` + SKIPPED | 실패 경로 없음 | 실패 콜백 + Sentry 추가 |
...
- **`android/app/src/main/java/com/becalm/android/ui/onboarding/OnboardingViewModel.kt`** —
  ...
  - 신규 `onOutlookFailed(error: OutlookAuthError)`: Sentry breadcrumb `onboarding_step_failed` (step="LINK_OUTLOOK_MAIL") + `onMarkStepStatus(LINK_OUTLOOK_MAIL, SKIPPED)`. (5.1 Gmail plan 과 동일 패턴 — VM 쪽 `sentryClient` inject 는 #12 에서 이미 완료된 상태 전제.)
...
- **tests**: `OnboardingViewModelTest` 에 두 케이스 추가
  ...
  - `onOutlookFailed_marksSkippedAndEmitsSentryBreadcrumb`
...
- [ ] **Unit test**: `OnboardingViewModelTest.onOutlookFailed_marksSkippedAndEmitsSentryBreadcrumb` 통과 — Sentry `onboarding_step_failed` (step="LINK_OUTLOOK_MAIL") + SKIPPED + `setOutlookMailConnected` 미호출
...
- **Blocked by (soft)**: #12 Gmail plan — 같은 `OnboardingViewModel` 의 sentryClient / oauthCredentialStore inject 를 먼저 도입. #13 이 혼자 실행되면 DI 추가가 중복됨.
- **Blocks**:
...
- **Sentry breadcrumb payload**: error code 는 MSAL 의 `MsalException.errorCode` 를 문자열로 넘김. 그 외 필드는 PIPA 상 사용자 식별자 포함 금지 (email 등).
