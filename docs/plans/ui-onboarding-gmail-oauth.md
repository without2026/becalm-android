# UI / Onboarding / gmail-oauth — GmailOAuthScreen 을 실제 Google Identity Services 흐름으로 교체 + 실패 시 Firebase Crashlytics `onboarding_step_failed`
...
3. **ONB-007 의 Firebase Crashlytics `onboarding_step_failed` 가 발생하지 않는다** — 거부·실패 자체가 코드 경로에 존재하지 않기 때문.
...
> "온보딩 실패 이벤트가 Firebase Crashlytics에 전송된다 … trigger: 온보딩 중 OAuth 인증 실패 또는 권한 거부 발생 … expected: Firebase Crashlytics에 `onboarding_step_failed` 이벤트 전송됨 (step 이름, error 포함)"
...
### 3.4 Firebase Crashlytics 호출 지점
- `grep -rn "onboarding_step_failed" android/app/src/main/` = 0 matches.
- `grep -rn "Firebase Crashlytics SDK" android/app/src/main/` 은 core/observability 계층에 있으나 OnboardingViewModel 에서는 호출 없음.
...
| 실패/거부 시 | Firebase Crashlytics `onboarding_step_failed` + LINK_GMAIL=SKIPPED | 실패 경로 자체 없음 | 실패 콜백 + Firebase Crashlytics breadcrumb 추가 |
...
- **`android/app/src/main/java/com/becalm/android/ui/onboarding/OnboardingViewModel.kt`** —
  ...
  - 신규 `onGmailFailed(error: GmailAuthError)`: `observabilityClient.addBreadcrumb("onboarding_step_failed", mapOf("step" to "LINK_GMAIL", "error" to error.code))` + `observabilityClient.captureMessage("onboarding_step_failed")` → `onMarkStepStatus(LINK_GMAIL, SKIPPED)`.
  - constructor 에 `OAuthCredentialStore`, `Firebase CrashlyticsClient` (또는 이미 있는 `Observability` 래퍼) inject.
...
- **tests**: `android/app/src/test/java/com/becalm/android/ui/onboarding/OnboardingViewModelTest.kt` 에 두 개의 케이스 추가
  ...
  - `onGmailFailed_marksSkippedAndEmitsFirebase CrashlyticsBreadcrumb`
  Fake `OAuthCredentialStore` + Fake `Firebase CrashlyticsClient` 사용. (kotest/junit4 + MockK 는 이미 프로젝트 convention.)
...
- [ ] **Unit test**: `OnboardingViewModelTest.onGmailFailed_marksSkippedAndEmitsFirebase CrashlyticsBreadcrumb` 통과 — Firebase Crashlytics breadcrumb `onboarding_step_failed` (step="LINK_GMAIL") emit + LINK_GMAIL=SKIPPED + `setGmailConnected` 호출 **안 됨**
...
3. `OAuthCredentialStore` / `Firebase CrashlyticsClient` inject 는 VM 에서 제거 필요 (revert 자동 처리).
...
- **Firebase Crashlytics breadcrumb vs captureMessage**: ONB-007 의 "이벤트 전송" 문구는 breadcrumb 만으론 부족 — `captureMessage("onboarding_step_failed", level=WARNING)` 도 동반 emit 해야 실제 event 로 Firebase Crashlytics UI 에 남음. 두 호출 모두 필수.
- **테스트 `Firebase CrashlyticsClient` 주입**: 프로젝트에 이미 `Observability` 또는 유사 래퍼가 있다면 그걸 inject. Fake 는 `recordedEvents: MutableList<String>` 로 단순 검증.
