...
- **화상회의(Zoom·Microsoft Teams·Google Meet) 회의 오디오·채팅·녹화·자막 수집**: 별도 플랫폼 OAuth·웹훅·자동 다운로드 통합이 필요하므로 MVP 제외. 캘린더 이벤트에 포함된 회의 링크(예: `meet.google.com/...`)는 calendar_events.source_ref/attendees_raw 수준에서만 보존되며 실제 회의 콘텐츠(오디오/트랜스크립트/채팅)는 수집되지 않는다. 사용자가 회의 내용을 수집하려면 Samsung Voice Recorder로 별도 녹음해야 함
- **다기기 동기화**: MVP는 단일 기기(삼성 주력 폰) 전용. 동일 계정으로 두 번째 기기 로그인 시의 Room mirror·cursor 공유·last-write-wins 충돌 해소는 post-MVP에 설계
...
- 🎥 화상회의 통합 (Zoom·Teams·Meet 오디오/채팅/자막 수집)
- 📱 다기기 동기화 (단일 계정 복수 기기 · last-write-wins conflict resolution · cursor sharing)
- 📊 **통계·분석 대시보드**: 기간별 give/take 비율, 이행률(completed/total), 평균 due 준수 지연, 상위 상대방 인터랙션 빈도 등 메타 지표. Room 집계 쿼리 기반이므로 추가 소스 없이 구축 가능
- 🔍 **전역 검색**: 모든 소스(음성·메일·캘린더) 통합 검색. quote 전문 + event_snippet + EmailBody body_plain 대상. Android FTS4 (Room @Fts4) 인덱스 기반. 필터(소스·기간·direction·person_ref) 결합 지원
- 🔒 **인물별 프라이버시 필터**: 특정 person_ref에 대해 "이 사람 메일 본문은 요약 금지"·"이 사람 통화 녹음 자동 추출 제외" 등 개별 규칙 설정. ENR-008의 연락처 pseudo-source 확장. 회사 기밀·민감 관계자 보호 목적

**MVP 스키마가 v2 기능의 기반** — `counterparty_raw`, `source_ref`, `action_state`, `raw_ingestion_events`, `person_ref` 필드가 이미 있어 마이그레이션 비용 최소.
