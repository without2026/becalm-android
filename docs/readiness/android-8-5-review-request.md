...
|---|---|---|
| Functional requirements | `Android Tests` run `25883105600`; `unit-tests`, `instrumented-tests`, and `release-smoke` jobs succeeded | Ready for reviewer |
...
| Readiness smoke result | Run `25883105600` measured cold start `2296ms`, total PSS `145095KB`, logcat fatal/ANR/OOM pass, and `readiness_failure_count=0` | Ready for reviewer |
| App install recovery | Run `25883105600` found the app missing after instrumentation, reinstalled `app-debug.apk`, and then completed readiness measurement | Ready for reviewer |
...
| Code quality | `Android Deterministic Gates` run `25883104187` succeeded with spec coverage, assert guard, secret detection, dependency-check task presence, Android lint, and size check | Ready for reviewer |
...
| Release smoke | `Android Tests` run `25883105600` succeeded for `assembleRelease lintRelease` and APK size smoke | Ready for reviewer |
| Instrumented tests | `Android Tests` run `25883105600` succeeded for API 33 emulator `connectedDebugAndroidTest` | Ready for reviewer |
