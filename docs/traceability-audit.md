...
`{feat|refactor|fix}/{layer}/{module}/{logic}`

- **layer**: `db` | `worker` | `ui` | `repo` | `docs`
- **module**: `voice` | `commitment` | `person` | `calendar` | `auth` | `onboarding` | `sms` | …
- **logic**: 단일 관심사를 나타내는 slug

같은 branch에 추가 fix가 들어오면 **새 branch 금지** — 열린 PR에 commit push. PR이 닫혔으면 같은 branch에 새 PR.
...
| 2 | 업로드 (VoiceUploadWorker → Railway multipart) | VOI-001/002/004/006/007 | ⏳ pending |
| 3 | Railway + Vertex AI 추출 | VOI-003 | ⏳ pending |
| 4 | Room commitments INSERT | ING 응답 처리 | ⏳ pending |
| 5 | CommitmentManagement 화면 렌더 | CMT-001~010 | ⏳ pending |
...
| # | Branch | Stage | Layer/Module | Plan doc | Status |
|---|--------|-------|--------------|----------|--------|
| — | `feat/docs/traceability-audit` | — | docs | 이 문서 + `_template.md` | 🚧 in review (this PR) |
| — | `feat/db/voice/call-recording-enum` | 1 | db/voice | `docs/plans/db-voice-call-recording-enum.md` | ⏳ not yet opened |
| — | `refactor/worker/sms/remove-dead-code` | 1 | worker/sms | `docs/plans/worker-sms-remove-dead-code.md` | ⏳ not yet opened |
| — | `refactor/worker/voice/ingestion-realign` | 1 | worker/voice | `docs/plans/worker-voice-ingestion-realign.md` | ⏳ not yet opened |
| — | `feat/worker/voice/call-recording` | 1 | worker/voice | `docs/plans/worker-voice-call-recording.md` | ⏳ not yet opened |

PR #이 부여되면 이 표에 업데이트됨.
